/**
 * mock.js —— 拟真生理数据发生器（无硬件时的数据源，CommonJS）
 *
 * 1Hz（setInterval 1000ms）推送样本，循环剧本（默认每周期 90 秒）：
 *   0–30s   静息基线：HR ~68±3、GSR ~1.2±0.1、RMSSD ~45±5
 *   30–60s  紧张：HR 升到 95+、GSR 升高、RMSSD 下降
 *   60–90s  恢复放松：RMSSD 超过基线、HR 回落
 * 阶段间用余弦平滑过渡（无阶跃跳变），叠加高斯噪声与呼吸性慢波动。
 *
 * 导出 start/stop 控制接口：createMockSource(onSample, opts) => { start, stop, reset, isRunning }
 */

// 各阶段目标值（按 mapping.js 校准记录 v3 的量程标定：
// 紧张 HR+24% / GSR+45% → a≈0.6；恢复 RMSSD+33% → v≈+0.55，
// 轨迹走满象限但不钉边）
const PHASES = [
...
/**
 * 创建 mock 数据源
 * @param {(sample:{hr:number,gsr:number,rmssd:number})=>void} onSample 每秒回调
 * @param {{cycleSec?:number}} opts cycleSec 可缩短周期用于加速自测
 */
function createMockSource(onSample, opts = {}) {
...
module.exports = { createMockSource };
